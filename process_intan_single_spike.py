...
def main():
    # 设置根目录
    input_root_folder = Path('/hy-tmp/min3d/min4d+3_240902_185321')
    target_rhd_file = input_root_folder / 'min4d+3_240902_201322.rhd'  # 替换为你想处理的 .rhd 文件名

    # 设置日志记录
    logging.basicConfig(
        filename=input_root_folder / 'processing.log',
        level=logging.INFO,
        format='%(asctime)s - %(levelname)s - %(message)s'
    )

    # 检查目标 .rhd 文件是否存在
    if not target_rhd_file.exists():
        print(f"指定的文件 {target_rhd_file} 不存在。")
        logging.info(f"指定的文件 {target_rhd_file} 不存在。")
        return
    else:
        print(f"找到文件 {target_rhd_file}。")
        logging.info(f"找到文件 {target_rhd_file}。")

    # 处理指定的 .rhd 文件
    process_rhd_file(target_rhd_file, input_root_folder)
