...
def process_rhd_file(rhd_file, input_root_folder, n_jobs=1):
    """
    处理单个 .rhd 文件的函数。
    ...
    try:   #####开始一个异常捕获块，以便在整个处理过程中捕获并记录任何错误。
        ...
        # 预处理步骤（带通滤波和参考）
        print("开始预处理")
        recording_f = si.bandpass_filter(raw_rec, freq_min=0.5, freq_max=200) ####检测动作电位需要  300  到3000Hz
        ...
        # 检测并移除坏通道
        print("检测并移除坏通道")
        bad_channel_ids, channel_labels = si.detect_bad_channels(recording_cmr, method='coherence+psd')  ##使用相干性和功率谱密度（PSD）方法进行坏通道检测
        recording_good_channels_f = recording_cmr.remove_channels(bad_channel_ids)  ##移除坏通道后的记录对象。
        recording_good_channels = si.common_reference(recording_good_channels_f, reference='global', operator='median')  ##使用全局参考。
        print("坏通道处理完成")
        logging.info("坏通道处理完成")

        # 设置并行处理参数
        job_kwargs_save = dict(n_jobs=n_jobs, chunk_duration="1s", progress_bar=True)  ##定义一个字典，包含以下参数：n_jobs：并行处理的CPU核心数，这里设置为 1。chunk_duration：数据处理的时间块大小，这里设置为 1 秒。progress_bar：是否显示进度条，这里设置为 True。
        ...
        recording_saved = recording_good_channels.save(folder=preprocessed_folder, **job_kwargs_save)
        print("预处理后的数据保存完成")
        ...
        #     # 不传递 job_kwargs_save   这个地方传递job=1会报错
        # )
        # print("排序器运行完成")
...
#
#     # 设置根目录
#     input_root_folder = Path(r'I:\test')
#
#     # 设置日志记录
...
#         process_rhd_file(rhd_file, input_root_folder, n_jobs=n_jobs)

def main():
    # 设置命令行参数解析
    parser = argparse.ArgumentParser(description="批量处理电生理 .rhd 数据文件。")
    parser.add_argument('folder', type=str, help="要处理的根文件夹路径。")
    parser.add_argument('--jobs', type=int, default=1, help="处理过程中使用的 CPU 核心数。默认为1。")
